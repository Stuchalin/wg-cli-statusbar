...
- Namer tests inject a fake file system; card and menu logic are tested through `StatusCardViewModel`, `StatusMenuStructure`, `StatusItemController.performStatusAction` and `StatusItemController.iconConnected(for:)` (no `NSStatusItem`/`NSHostingView` in tests).
