...
- `WGStatusBarCore` — library target split across `Sources/WGStatusBarCore/`, shared by both executables (the version constants compile into both so they cannot drift):
    ...
    - `WGBinaryResolver.swift` — finds `wg` in `/opt/homebrew/bin`, `/usr/local/bin`, `/usr/bin` (launchd has no user PATH); caches only successful resolves and revalidates the cached path each call, so both a later `brew install` and a `brew uninstall` are picked up without a daemon restart.
