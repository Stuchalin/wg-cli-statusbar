...
- `WGStatusBarCore` — library target split across `Sources/WGStatusBarCore/`, shared by both executables (the version constants compile into both so they cannot drift):
    ...
    - `WGBinaryResolver.swift` — finds `wg` in `/opt/homebrew/bin`, `/usr/local/bin`, `/usr/bin` (launchd has no user PATH); caches only successful resolves so a later `brew install` is picked up without a daemon restart.
    - `WGShowExecutor.swift` — daemon-side runner: literal args `show all dump`, ~4 s timeout (hangs are killed), `WGShowExecutorError` (`wgMissing`/`timedOut`/`wgFailed`); returns raw output — sanitizing is not its job.
    - `HelperDaemon.swift` — `DaemonServer`: unlink+bind the socket (0660 root:admin best-effort), sequential accept-loop, one connection = one request, read deadline for silent clients, sanitizes the executor output before replying (the single point where secrets are stripped).
    ...
    - `InstallerService.swift` — daemon install/uninstall via osascript `do shell script ... with administrator privileges`; static pure functions (script resolve, argv build, exit-code interpretation) are the tested part; the prompted run is not automatable.
...
- Paths: socket `/var/run/wgstatusbar.sock` (0660 root:admin; the server unlinks a stale file before binding); helper binary `/Library/PrivilegedHelperTools/com.stuchalin.wgstatusbar.helper` (755 root:wheel); plist `/Library/LaunchDaemons/com.stuchalin.wgstatusbar.helper.plist` (`RunAtLoad` + `KeepAlive`).
- Install scripts: `scripts/install-daemon.sh --binary <path>` (idempotent: bootout ignoring "not loaded" → cp → bootstrap system with a short retry) and `scripts/uninstall-daemon.sh` (bootout → rm plist, binary, socket); both ship in the .app's `Contents/Resources/` (`build-app.sh` also copies the helper binary to `Contents/MacOS/`).
...
- Accepted risk (documented in README): the install path runs a root shell script from the user-writable app bundle with no checksum (TOCTOU); the real fix is Developer ID + SMAppService, a future migration the protocol is designed for (transport change only).
