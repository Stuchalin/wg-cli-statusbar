...
- Install scripts: `scripts/install-daemon.sh --binary <path>` (idempotent: bootout ignoring "not loaded" → copy to a temp name + atomic rename over the live binary, so an async-bootout old daemon is not truncated under a running process → bootstrap system with a short retry) and `scripts/uninstall-daemon.sh` (bootout → rm plist, binary, socket, log); both ship in the .app's `Contents/Resources/` (`build-app.sh` also copies the helper binary to `Contents/MacOS/`).
