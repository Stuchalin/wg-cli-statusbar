...
/// запрос `config` (маскированный просмотр конфига); билд 19 — one-shot-режим
/// бинаря (`--capabilities`, `--print-config-raw`): протокол сокета не менялся,
/// но префлайт Reveal обязан отсечь бинарь без этих флагов — билд 18 на пробе
/// `--capabilities` просто стартовал бы второй демон.
public let helperBuildNumber = 19
...
enum ConfigEnvelopeError: Error, Equatable {
    ...
    case extraLines
    /// Закодированная часть — не валидный или не каноничный base64 (алфавит,
    /// `=` в середине, длина не кратна 4, ненулевые неиспользуемые биты
    /// паддинга — напр. `AB==`).
...
enum ConfigEnvelope {
    ...
    /// конверта, требует тег, одну строку, валидный каноничный base64, размер
    /// в пределах `maxDecodedBytes` и полный UTF-8-декод. Возвращает точный текст
    ...
    static func decode(
        ...
        guard isWellFormedBase64(encoded),
              ...
              // Каноничность: ре-кодирование декодированных байтов обязано
              // байт-в-байт воспроизводить вход. Неканоничные хвосты (`AB==`,
              // `AAB=` — ненулевые неиспользуемые биты паддинга) Foundation
              // декодирует теми же байтами, что каноничные `AA==`/`AAA=`, но
              // наш кодировщик порождает только каноничную форму — всё
              // неканоничное на этом канале мусор, а не документ.
    ...
    /// местами снисходителен — конверт проверяем своей формой. Каноничность
    /// (нулевые неиспользуемые биты паддинга) здесь не проверяется: её в
    /// `decode` закрывает сравнение входа с ре-кодированием.
