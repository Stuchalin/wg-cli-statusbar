...
final class HelperDaemonTests: XCTestCase {
    ...
    /// Заглушка исполнителя: конфигурируемый дамп/ошибка/задержка + счётчик вызовов.
    ...
    @discardableResult
    private func performExchange(_ request: String?) throws -> (response: String, elapsed: TimeInterval) {
    ...
    func testClientDisconnectedBeforeResponseDoesNotKillLoop() async throws {
        let executor = StubExecutor()
        // Задержка: клиент закрывается раньше, чем сервер пишет ответ (EPIPE вместо SIGPIPE).
        executor.configure(dump: "dump", delay: 0.15)
        try await startServer(executor: executor)

        try connectSendAndClose(encode(.show))
        try await waitFor { executor.callCount == 1 }

        // Цикл жив после записи в мёртвый сокет: следующий клиент обслуживается.
        let exchange = try performExchange(encode(.show))
        let response = try XCTUnwrap(
            decode(response: exchange.response),
