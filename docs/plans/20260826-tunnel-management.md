...
| helperBuildNumber | 8 → 13 | Task 1: 9; ревью-фикс: 10; ➕-ревью Task 4: 11, 12, 13 |
...
- Обновить демон через меню (Update) — актуальный `helperBuildNumber` (13 на момент ревью; висяк `wait` закрыт только в build 11+, ранние build'ы проверять бессмысленно); убедиться, что после обновления секция Tunnels появилась
