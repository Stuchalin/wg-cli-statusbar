...
- [x] Implement a fresh `.deviceOwnerAuthentication` evaluation per Reveal with Touch ID reuse duration zero and localized reason/cancel text.
- [x] Add a preparation/preflight step that checks service state first, then validates the name, verifies the fixed directory/file ownership and mode, and runs the exact installed binary unprivileged with `--capabilities`; require current protocol, build `>= helperBuildNumber`, and `config-raw-v1` before creating an authentication context or launching any privileged process. `.absent`, `.broken`, and `.outdated` must return viewer-local Install/Update guidance before the filesystem/capability/authentication/process boundaries.
- [x] Classify success, user cancellation, unavailable policy, and other failures without treating cancellation as an app error.
- [x] Add a pure core argv parser with explicit daemon, capabilities, raw-read, and invalid results. Unit-test no arguments, exact `--capabilities`, exact `--print-config-raw <name>`, missing/extra arguments, and unknown flags; keep `Sources/Helper/main.swift` as a thin executor of that result.
- [x] Implement the side-effect-free capability result and `HelperOneShotMode` raw result so the latter writes one `b64:<base64>\n` envelope only on a complete safe read; leave normal no-argument daemon startup unchanged.
- [x] Implement safely quoted `osascript` argv, chunked concurrent stdout/stderr drain with accumulation caps, tagged/base64 decode, cancellation with bounded process termination, launch failure, exit failure, and no raw data in diagnostics.
- [x] Prevent duplicate Reveal operations and ensure cancellation/failure returns no raw document.
- [x] Test `.absent`, `.broken`, and `.outdated` service states separately: each produces the expected viewer-local Install/Update guidance with zero filesystem/capability calls, zero authentication-context creation, and zero privileged-process launches.
- [x] With daemon state current, test unsafe parent directory; missing/symlink/non-regular/non-root-owned/group-or-world-writable/non-executable helper; capability launch failure, timeout, oversize, malformed output, wrong protocol, stale build, and missing capability. Every preflight failure must make zero authentication/privileged-process calls. Also test shell/AppleScript injection strings, every privileged process outcome, empty/invalid/truncated/oversized base64, authentication branches, and success content.
- [x] Acceptance: all non-installed service states fail closed before any prompt or process; helper argv dispatch is exhaustively unit-tested in core; raw configuration is obtainable only through the explicit authenticated one-shot path using the installed root-owned helper; the user-writable bundled copy is never a Reveal execution target; no daemon socket request can return raw content; helper stderr and returned errors contain fixed categories only.
- [x] Validation: run `PrivilegedConfigReaderTests`, `HelperOneShotTests`, `InstallerServiceTests`, then `git diff --check`.
- [x] run tests - must pass before next task
...
- [x] Run every focused validation command and capture the full gate with `set -o pipefail; make test 2>&1 | tee /private/tmp/wgstatusbar-config-viewer-tests.log` so a failing test cannot be hidden by `tee`. (Evidence: all seven focused suites green — TunnelConfigReaderTests 21, ConfigSanitizerTests 12, ConfigSocketTests 15, PrivilegedConfigReaderTests 56, ConfigViewerModelTests 17, ConfigViewerLocalizationTests 2, StatusItemControllerTests 24; full gate via tee: 484 tests, 0 failures, pipefail exit 0, log at `/private/tmp/wgstatusbar-config-viewer-tests.log`.)
- [x] Set `secret_canary='WGSTATUSBAR_SECRET_CANARY_7F2E9C41'`; require exactly one `rg -n -F "$secret_canary" Sources Tests` match, in `ConfigSecretFixture.swift`, and zero `rg -a -F` matches in `build/WGStatusBar.app` and the captured test log. (Evidence: count = 1, sole match `Tests/WGStatusBarTests/ConfigSecretFixture.swift:9`; zero matches in the release bundle and in the captured test log.)
- [x] Verify daemon round trips mask canonical key-assignment values, preserve the other documented full-text content, and cannot reach the unsanitized one-shot mode through daemon request parsing. (Evidence: `ConfigSocketTests.testConfigRoundTripReturnsSanitizedDocumentAndCanaryNeverCrossesDaemon` plus envelope/newline/empty-document and error-content cases, `HelperDaemonTests.testConfigReturnsSanitizedEnvelope`/`testConfigPreservesDocumentWithoutFinalNewline`/`testConfigRoundTripsEmptyDocument` — all green in the full gate; code inspection: `HelperDaemon.handleClient` routes only `show`/`list`/`state`/`up`/`down`/`config` and answers unknown commands with `err`, while `--print-config-raw` is reachable only through helper argv (`parseHelperArgv` in `Sources/Helper/main.swift`), never through the socket; `HelperProtocolTests.testEncodeIsExhaustiveOverRequestCases` pins that no raw request exists.)
- [x] Build the `.app` with `make release`; inspect the bundle to confirm the expected helper binary and localization resources are present. (Evidence: bundle contains `Contents/MacOS/WGStatusBarHelper`, install/uninstall scripts, `AppIcon.icns`, and `WGStatusBar_WGStatusBarCore.bundle/{en,ru}.lproj/Localizable.strings`; ad-hoc signed; the bundled helper answers unprivileged `--capabilities` with `capabilities 1 19 config-raw-v1` — protocol stays 1, build advanced to 19, at or above the plan's minimum 18.)
- [x] With explicit user authorization, update the local helper, create a dedicated synthetic QA config containing the canary, and manually verify masked open, fresh Touch ID/password flow, administrator fallback if presented, cancellation, Reveal/Hide/Reload, copy, file change, window close during auth, themes, both locales, and VoiceOver; remove the QA config afterward. (Not performed — requires explicit user authorization for helper install/update, Touch ID/password/admin approval and a QA config on the real machine; recorded as incomplete per plan, never as automated success.)
- [x] Immediately before QA, set `qa_log_start="$(date '+%Y-%m-%d %H:%M:%S')"` and record `helper_log_offset="$(sudo stat -f %z /var/log/wgstatusbar-helper.log)"`. After QA, write `/usr/bin/log show --start "$qa_log_start" --predicate 'process == "WGStatusBar"' --style compact` to `/private/tmp/wgstatusbar-config-viewer-app.log` and `sudo tail -c "+$((helper_log_offset + 1))" /var/log/wgstatusbar-helper.log` to `/private/tmp/wgstatusbar-config-viewer-helper.log`; require zero `rg -F "$secret_canary"` matches in both. Reading the root-owned helper log and creating/removing the QA config require explicit authorization. (Not performed — same authorization gap: reading the root-owned helper log and creating the QA config are explicit manual actions per plan; no log-capture artifacts were produced or claimed.)
- [x] Record manual steps that cannot run as incomplete with the exact reason; never convert missing authorization or environment into success. (Recorded: the two QA items above are incomplete because no user authorization for privileged helper install/update, system authentication, root-owned log access, or QA-config creation exists in this automated run; recorded here and in `.ralphex/progress/progress-20260903-wireguard-config-viewer.txt`.)
- [x] Acceptance: all automated checks pass; every performed manual scenario matches the approved design; leakage of canonical key-assignment values into default transport/logs or leakage of the revealed document into logs/artifacts is blocking. (Automated: all checks above pass and no canary leakage appears in any automated artifact — response bytes, parsed output, bundle, test log; manual scenarios: none performed — see the two incomplete QA items; no leakage claim extends to the unperformed manual flows.)
- [x] Validation: the captured `make test` pipeline, `make release`, the exact source/bundle/test-log canary commands from `Validation Commands`, `git diff --check`, plus the recorded manual matrix and authorized app/helper-log canary searches. (All automated parts executed and green; the manual matrix and the authorized app/helper-log canary searches remain pending user-authorized QA.)
- [x] run tests - must pass before next task
...
- [x] Document how to open the viewer, masked-by-default behavior, Reveal/Hide/Reload, read-only scope, system authentication, possible additional administrator prompt, and old-helper update behavior. (README: new "Config viewer" section + a Features bullet — document button on each tunnel row, `(hidden)` masking by default, fresh Touch ID/Apple Watch/password check per Reveal with the possible second admin prompt and silent cancellation, Hide/Reload/close/switch semantics, read-only scope, stale-helper install/update guidance with no prompt.)
- [x] Update architecture and wire-protocol documentation for the masked config request, helper build, shared safe reader, one-shot mode, viewer ownership, errors, and test boundaries. (AGENTS.md: overview + thin-dispatcher helper main + AppDelegate ownership wiring; new Core bullets — `TunnelConfigReader`, `ConfigSanitizer`, `HelperOneShotMode`, `SocketConfigClient`, `PrivilegedConfigReader`, `ConfigViewer`; updated `HelperProtocol` (config request, `config-unavailable`, `ConfigEnvelope`, build 17/18/19 history), `TunnelConfigStore` (`isNameShapeValid`), `HelperDaemon` routing/init, `HelperClient` `responseLimit`, defensive mappings in both legacy clients, `TunnelRowView` dual hit targets, `StatusItemController.presentConfigViewer`; new "Config viewer" data-flow subsection; wire-protocol and budget-invariant updates; config-viewer test-suite paragraph; manual-QA viewer items.)
- [x] State the security limits: unauthenticated masked view hides only canonical `PrivateKey`/`PresharedKey` assignment values; comments/hooks/unknown directives remain visible; raw text enters app memory after reveal; Swift memory is not guaranteed zeroed; clipboard persists; ad-hoc signing and the install-time bundle TOCTOU remain. Reveal itself executes only the installed root-owned helper. (README "Security notes": the dump promise separated from the narrowed config-viewer promise, memory/clipboard limits, one-shot-only raw path via the root-owned installed helper with preflight, no raw socket command; AGENTS "Security constraints": same limits plus fixed-category errors and the synthetic-canary rule. Ad-hoc signing and TOCTOU were already stated in both files and remain unchanged.)
- [x] Keep examples synthetic and never paste a real configuration or key into repository files. (Docs use only the `(hidden)` placeholder; the canary literal was not copied into README/AGENTS — `rg` over both finds zero occurrences; the sole literal under `Sources`/`Tests` remains `Tests/WGStatusBarTests/ConfigSecretFixture.swift:9`.)
- [x] Acceptance: user and agent documentation describe the implemented behavior and its verified limits without claiming unperformed manual authentication. (Both files describe behavior and design only; the manual matrix stays in AGENTS "Manual QA checklist" as instructions, and Task 5's record of unperformed manual QA is unchanged.)
- [x] Validation: documentation searches, relevant doc tests if added, `make test`, and `git diff --check`. (Topic searches green — README: Reveal/Hide/Reload/masked/read-only/administrator/stale-helper; AGENTS: `config <name>`, `config-unavailable`, `b64:`, `--print-config-raw`, `--capabilities`, `isNameShapeValid`, `TunnelConfigReader`, `sanitizeWGQuickConfig`, `PrivilegedConfigReader`, `ConfigViewerModel`, `.deviceOwnerAuthentication`, build 19, `ConfigSecretFixture`, `presentConfigViewer`; no doc tests added; full gate re-run with `set -o pipefail; make test 2>&1 | tee /private/tmp/wgstatusbar-config-viewer-tests.log` → 484 tests, 0 failures, exit 0; canary absent from the fresh log; `git diff --check` clean.)
- [x] run tests - must pass before next task
...
### Final status (2026-09-03)

Completed on branch `feat/config-viewer`, commits `df31918`…Task 6. Helper protocol stays `1`; `helperBuildNumber` advanced 17 → 18 (masked `config` request) → 19 (helper one-shot modes `--capabilities` / `--print-config-raw`); a build-18 helper is deliberately rejected by the Reveal preflight. Automated evidence: full gate 484 tests, 0 failures (`/private/tmp/wgstatusbar-config-viewer-tests.log`), canary exactly one literal under `Sources`/`Tests` and absent from the log and the release bundle, `make release` bundle inspected (Task 5). Documentation: README "Config viewer" + security notes; AGENTS.md architecture, wire protocol, security constraints, testing notes, manual QA. Manual authentication/QA flows (Touch ID/password/admin prompt, real-config viewer runs, app/helper-log canary capture) remain **not performed** — they require explicit user authorization per plan; see the two incomplete QA items in Task 5 and the "Config viewer" entries in the AGENTS.md manual QA checklist.
