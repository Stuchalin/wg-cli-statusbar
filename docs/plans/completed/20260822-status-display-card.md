...
- [x] тест: во временной директории `work-vpn.name` с содержимым `utun2` + `utun2.sock` → резолв `utun2` → `work-vpn`; кейс `utun10` (6 байт + перевод строки в конце)
- [x] тест: нет `.sock` рядом → запись считается устаревшей, fallback на сырое имя
- [x] тест: каталог не существует / пуст → fallback; несколько `.name` файлов → каждый мапится на свой utun
- [x] тест кэша: повторный `displayName(for:)` без изменений не читает файловую систему (инжект-фс/счётчик чтений)
- [x] реализация: сканирование `<dir>/*.name`, чтение содержимого целиком + trim переводов строк (без фиксации длины), кэш `utun → имя` под `NSLock` (вызовы из фонового refresh и главного потока по ⌘R), инжект пути (по умолчанию `/var/run/wireguard`)
  - ⚠️ отклонение (ревью Codex 2026-08-22): попадание в кэш валидируется по текущей паре `.name`/`.sock` (чтение одного файла + проверка сокета, без листинга каталога) — macOS переиспользует номера utun, и без валидации карточка показывала бы имя прошлого конфига до ручного ⌘R; невалидная запись — исключительный rescan
  - ⚠️ отклонение (ревью Codex 2026-08-22, итерация 2): пара `.name`/`.sock` дополнительно проверяется по mtime (|Δ| < 2 c — правило `get_real_interface` из darwin.bash, оба файла создаёт wireguard-go при подъёме туннеля): сокет персональный для интерфейса, и если старый `.name` завис, а utun занял другой конфиг, содержимого и существования сокета недостаточно — имя не должно пережить пересоздание сокета; проверка применяется и к кэшу, и к скану (иначе выбор между двумя `.name` на одном utun зависел бы от порядка листинга каталога)
- [x] `swift test` — зелёные
...
- [x] интеграция namer в `refresh()`: ленивый рескан только для незнакомых utun + принудительный по кнопке Обновить (параметр refresh)
- [x] `swift test` — зелёные
...
- [x] все требования Overview реализованы (карточка, цвета, имена, пункты меню с шорткатами)
- [x] краевые случаи: пустой вывод wg, ошибка команды, неизвестный utun, пиров 0/1/много
- [x] полный прогон: `swift test`
- [x] сборка release: `swift build -c release`
